...
async def start(client, message):
    ...
    if id:
        if old == True:
            ...
            await client.send_message(id, "Congrats! You Won 1.2GB Upload limit")
...
async def send_doc(client, message):
    ...
    if update_channel:
        try:
            ...
            await client.send_message(log_channel,f"🦋 #Dany_rename_bot 🦋,\n\n**ID** : `{user_id}`\n**Name**: {message.from_user.first_name} {message.from_user.last_name}\n**User-Plan** : {user}\n\n ",
                                                                                                       reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔺 Restrict User ( Restrict Power ) 🔺", callback_data="ceasepower")]]))
            return
