...
async def buypremium(bot, message):
	await message.reply_text("🦋 Select Plan to upgrade...", quote=True, reply_markup=InlineKeyboardMarkup([
		           [
				   InlineKeyboardButton("🥉 Bronze - 10GB 🥉", callback_data="vip1")
				   ],[
					InlineKeyboardButton("🥈 Silver - 25GB 🥈", callback_data="vip2")
				   ],[
					InlineKeyboardButton("🪙 Gold - 40GB 🪙", callback_data="vip3")
				   ],[
					InlineKeyboardButton("✨ Platinum - 70GB ✨", callback_data="vip4")
				   ],[
					InlineKeyboardButton("💎 Diamond - 150GB 💎", callback_data="vip5")
					]]))
...
async def resetpower(bot, message):
	    await message.reply_text(text=f"Do you really want to reset daily limit to default data limit 3 GB ?",quote=True,reply_markup=InlineKeyboardMarkup([
...
async def vip2(bot,update):
	...
	addpre(int(user_id))
	await update.message.edit("Added successfully To Silver Premium Upload limit 25 GB")
...
async def vip3(bot,update):
	...
	addpre(int(user_id))
	await update.message.edit("Added successfully To Gold Premium Upload limit 40 GB")
...
async def vip4(bot,update):
	...
	user_id = id[1].replace(" ", "")
	inlimit = 75161927680
	uploadlimit(int(user_id), 75161927680)
...
async def vip5(bot,update):
	id = update.message.reply_to_message.text.split("/addpremium")
	user_id = id[1].replace(" ", "")
	inlimit = 161061273600
	uploadlimit(int(user_id), 161061273600)
